...
export function useGameActions() {
  ...
  const startRound = useCallback(async (mode: 'A' | 'B' = 'A') => {
    if (!socket) return;
    ...
    try {
      if (mode === 'A') {
        const response = await fetch('http://localhost:3001/game/mode1/question');
        gameData = await response.json();
        correctAnswer = gameData.correctProfessorId;
        options = gameData.professorOptions.map((p: any) => p._id);
      } else {
        const response = await fetch('http://localhost:3001/game/mode2/question');
        gameData = await response.json();
