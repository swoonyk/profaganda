  ...
  useEffect(() => {
    ...
    socket.on('server:players_update', ({ players }) => {
      const updatedPlayers = players.map((player: any) => ({
        ...player,
        yourself: player.name === playerIdRef.current || player.playerId === playerIdRef.current
      }));
    ...
    socket.on('server:answer_ack', ({ roundId, accepted, correct, points }) => {
      if (accepted) {
        console.log(`Answer submitted successfully for round ${roundId}. Correct: ${correct}, Points: ${points}`);
      } else {
